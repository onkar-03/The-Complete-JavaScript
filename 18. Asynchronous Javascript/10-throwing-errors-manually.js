...
const getJson = function (url, errMsg = 'Something is Wrong!!') {
  // Fetch Data from API
  // As we created a new function we need to return the promise here explicity
  return (
    fetch(`${url}`)
      // Converting the resolved value of promise to JavaScript object using .json()
      .then(response => {
        // Checking ok status of Response
        // The Response we get From the API when promise fulfilled has an ok value being true/false
        // Status 200 means ok:true
        // Status 404 means ok:false
        // In case the API fails to retrieve data from the server for the Country passed then the OK:false is set
        if (!response.ok) {
          // throw is used to reject the Promise immediately / terminate the current function just like return works
          // The effect of using the throw is that the Promise will be immediately rejected means that the promise generated by this .then() method will be rejected
          // Aas .then() is used to handle only fulfilled promises hence the rejected promise will skip all the then handlers and propagate down to the catch function to display the Error message set here
          // Error() is a constructor used to Specify Error messages
...
// --- Old code for same implementation without function:
// Fetch Country Data
// const getCountryData = function (country) {
//   // Fetch Data from API
